"""
BYBIT FUTURES BOT - Render deployment
"""
...
CONFIG = {
    ...
    "rsi_high": 60,
    "scan_interval": 15,
    "pairs": ["BTCUSDT","ETHUSDT","SOLUSDT","BNBUSDT","XRPUSDT","DOGEUSDT","ADAUSDT","AVAXUSDT","LINKUSDT","DOTUSDT","LTCUSDT","UNIUSDT","ATOMUSDT","ETCUSDT","APTUSDT","ARBUSDT","OPUSDT","INJUSDT","SUIUSDT","FETUSDT","SEIUSDT","WLDUSDT","TIAUSDT","NEARUSDT","BNXUSDT"],
    "timeframe": "5",
}
...
def get_balance():
    try:
        ...
        log("error", f"get_balance retCode: {r['retCode']} {r['retMsg']}")
        return None
    except Exception as e:
...
def get_klines(symbol):
    try:
        r = STATE["session"].get_kline(category="linear", symbol=symbol, interval=CONFIG["timeframe"], limit=100)
        if r["retCode"] == 0:
            return [float(k[4]) for k in reversed(r["result"]["list"])]
        return None
    except Exception as e:
...
def set_leverage(symbol):
    try:
        STATE["session"].set_leverage(category="linear", symbol=symbol, buyLeverage=str(CONFIG["leverage"]), sellLeverage=str(CONFIG["leverage"]))
    except:
        pass

def place_order(symbol, side, qty, sl=None, tp=None):
    try:
        params = {"category": "linear", "symbol": symbol, "side": side, "orderType": "Market", "qty": str(qty), "timeInForce": "GoodTillCancel"}
        if sl:
...
def close_position(symbol, qty, side):
    try:
        ...
        STATE["session"].place_order(category="linear", symbol=symbol, side=close_side, orderType="Market", qty=str(qty), reduceOnly=True, timeInForce="GoodTillCancel")
    except Exception as e:
...
def get_indicators(symbol):
    ...
    return {"price": closes[-1], "rsi": calc_rsi(closes), "ema_fast": ef, "ema_slow": es, "trend": "BULL" if ef > es else "BEAR"}
...
def compute_signal(ind):
    ...
    if ind["rsi"] < CONFIG["rsi_low"] and bull:
        return "BUY"
    if ind["rsi"] > CONFIG["rsi_high"] and not bull:
        return "SELL"
    return "HOLD"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ["/", "/index.html"]:
            try:
                ...
                self.send_html("<h1>Bot en ligne</h1><p>dashboard.html introuvable</p>")
            return
        if path == "/api/state":
            total = STATE["wins"] + STATE["losses"]
            self.send_json({
                "running": STATE["running"], "mode": STATE["mode"],
                "pnl": round(STATE["pnl"], 4), "balance": round(STATE["balance"], 2),
                "positions": STATE["positions"], "signals": STATE["signals"],
                "indicators": STATE["indicators"], "logs": STATE["logs"][:50],
                "wins": STATE["wins"], "losses": STATE["losses"],
                "trades": STATE["trades"][:20],
                "win_rate": round(STATE["wins"]/total*100) if total > 0 else 0,
    ...
    def do_POST(self):
        ...
        if path == "/api/test_connection":
            ...
            if not PYBIT_OK:
                self.send_json({"ok": False, "msg": "pybit non installe"})
                return
            if not key or not secret:
                self.send_json({"ok": False, "msg": "Cles manquantes"})
                return
            try:
                session = BybitHTTP(
                    testnet=testnet,
